...
export class DataSource {
  ...
  constructor(
    ...
    this.rowsSubject = new BehaviorSubject<any[]>([]);
    ...
    this.pageInfo
      .debounceTime(300)
      .switchMap(pageInfo => this.read(pageInfo))
  ...
  public remove(row) {
    var req = this.resource.remove(row._id);
    req.subscribe(
      () => {
        var rows = this.rowsSubject.getValue()
  ...
  private read(pageInfo: PageInfo): Observable<QueryResult> {
  ...
  private onRead(data) {
    if (this.options.paginate) {
      this.rowsSubject.next(data.docs);
